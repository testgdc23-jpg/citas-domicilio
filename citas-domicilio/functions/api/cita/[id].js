...
export async function onRequest(context) {
  ...
  async function exists(table, id) {
    const row = await env.DB.prepare(`SELECT 1 AS ok FROM ${table} WHERE id = ?`)
      .bind(id)
      .first();
    return !!row?.ok;
  }
  ...
  if (request.method === "GET") {
    const row = await env.DB
      .prepare(
        `SELECT id, paciente_id, direccion_id, seguro_id, fecha, hora,
  ...
  if (request.method === "PUT") {
    ...
    try {
      const res = await env.DB
        .prepare(
          `UPDATE cita
             SET paciente_id=?, direccion_id=?, seguro_id=?,
                 fecha=?, hora=?, sintomas=?, asistencia=?
           WHERE id=?`
        )
        .bind(
          paciente_id, direccion_id, seguro_id,
          fecha, hora, sintomas || null, asistencia ?? 0,
          citaId
        )
        .run();
  ...
  if (request.method === "DELETE") {
    const res = await env.DB
      .prepare("DELETE FROM cita WHERE id = ?")
      .bind(citaId)
      .run();
